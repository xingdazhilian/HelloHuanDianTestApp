 ...
 * Description: 电池策略表
 */
public final class BatteryStrategyTable
{
    ...
    public static final Map<Integer, byte[]> CMDS = new TreeMap<>();
     ...
     * 校验和填充
     */
    private static void checkSumFull()
    {
        byte[] v;
        for (Map.Entry<Integer, byte[]> entry : CMDS.entrySet())
        {
            v = entry.getValue();
